package com.zzhoujay.richtext.mathdisplay.parse
...
open class MTMathAtomFactory {
        ...
        if (matrixEnvs.containsKey(env)) {
            ...
            if (table.numColumns() != 2) {
                error.copyFrom(
                    MTParseError(
                        MTParseErrors.InvalidNumColumns,
                        "cases environment can only have 2 columns"
                    )
                )
                return null
            }
    ...
    private val supportedLatexSymbols: HashMap<String, MTMathAtom> = hashMapOf(
        ...
        // 加在这里没有任何作用
        "blacksquare" to MTMathAtom(MTMathAtomType.KMTMathAtomRelation, MTSymbolBlackSquare),
